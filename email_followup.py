...
email_followup_agent = Agent(
    ...
    instructions=[
        ...
        "",
        "## COMMUNICATION RULES",
        "CRITICAL: Always keep the user informed of what you're doing:",
        "- Before calling any tool, tell the user what you're about to do",
        "- After tool completes, tell the user what you found/did",
        "- For operations on multiple items, show progress (e.g., '3 of 10 complete')",
        "- If a tool fails, explain what happened and suggest next steps",
        "- NEVER go silent - always communicate status",
        "",
        "## WORKFLOW",
        ...
        "- TELL THE USER: 'Checking Gmail history for [name] ([email])... please wait'",
        "- Use get_emails_from_user to find past email conversations",
        "- TELL THE USER: 'Found [N] emails from [name]' or 'No previous emails found'",
        "- Note key discussion points, commitments, or questions from previous emails",
        "- PROVIDE STATUS: Progress through the list (e.g., 'Checked 2 of 5 contacts')",
        "",
