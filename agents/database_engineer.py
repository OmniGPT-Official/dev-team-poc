...
# Custom compression manager: only compress after 6 tool results accumulate in context
...
database_engineer_agent = Agent(
    ...
    num_history_messages=5,               # Prevent context explosion over long sessions
    compress_tool_results=True,           # Enable tool result compression
    compression_manager=_compression_manager,  # Compress after 6 results (not default 3)
    max_tool_calls_from_history=15,       # Cap historical tool calls loaded into context
    enable_session_summaries=True,        # Auto-generate session summaries
    ...
    skills=Skills(loaders=[LocalSkills(str(skills_dir))]),  # Load all skills (database-schema-design, database-migration)
