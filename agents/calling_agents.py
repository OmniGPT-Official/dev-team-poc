...
lead_reader_agent = Agent(
    ...
    instructions=[
        ...
        "Then a JSON array on a single line with ALL columns needed for calling:",
        '[{"phone_number":"+1234567890","language":"en","restaurant_name":"Joe\'s","city":"NYC","country":"US"},...]',
        "Include: phone_number, language, restaurant_name, city, country — omit only email, website, status.",
...
calling_coordinator_agent = Agent(
    ...
    instructions=[
        ...
        "When you receive the lead list from Step 1, pass ALL fields from every lead dict directly to submit_batch_call.",
        "The recipients list must include every field from every lead dict that Step 1 outputs — do NOT hardcode or filter fields.",
        "Pass phone_number, language, restaurant_name, city, country, email, website, and any other fields as-is.",
        "The submit_batch_call tool handles all API formatting — you just pass the full dicts.",
        "Example: if Step 1 outputs [{\"phone_number\":\"+66...\",\"language\":\"th\",\"restaurant_name\":\"Pad Thai\",\"city\":\"Bangkok\"}], pass that exact list as recipients.",
        "",
